...
// from the boolean/enum results). Ported 1:1 from SoloMapling (MovementCommands equivalents inlined).
final class GrindBreakRoutine {
        ...
        if (!restStarted) {
            ...
            if (restKind == RestSpotFinder.Kind.GROUND) {
                ...
                if (rng.nextDouble() < BREAK_SIGN_CHANCE) {
                    setChalkboard(chr, bot.breakSignText());
                    signSet = true;
                }
            }
    ...
    // ── MovementCommands equivalents (gms has no recorded-path engine) ──────────
    ...
    private static void setChalkboard(Character chr, String msg) {
        if (chr == null) {
            return;
        }
        chr.setChalkboard(msg);
        if (chr.getMap() != null) {
            chr.getMap().broadcastMessage(PacketCreator.useChalkboard(chr, false));
        }
    }

    private static void clearChalkboard(Character chr) {
        if (chr == null) {
            return;
        }
        chr.setChalkboard(null);
        if (chr.getMap() != null) {
            chr.getMap().broadcastMessage(PacketCreator.useChalkboard(chr, true));
        }
    }
