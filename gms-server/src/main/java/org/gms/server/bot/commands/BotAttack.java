...
import org.gms.server.bot.attack.BotAttackData;
...
public final class BotAttack {
    ...
    public static void basicSwing(Character chr) {
        ...
        int facingMask = chr.isFacingLeft() ? BotAttackData.FACING_LEFT_MASK : BotAttackData.FACING_RIGHT_MASK;
        WeaponType weaponType = resolveEquippedWeaponType(chr);
        int bodyActionId = BotAttackData.randomActionFor(weaponType);

        // gms 移植：SoloMapling 的 PacketCreator.*Attack 接受 Map<Integer, AttackTarget>；
        // gms 接受 Map<Integer, List<Integer>>，空目标集合直接泛型化为后者。
