...
import static org.gms.server.bot.itempool.UniqueStatBonusList.applySellablesDeduction;
import static org.gms.server.bot.itempool.UniqueStatBonusList.correctAnomalies;
import static org.gms.server.bot.itempool.ItemSelector.getScrollNodeData;
import static org.gms.server.bot.itempool.ScrollInfoManager.getScrollInfo;

/**
 * 移植自 SoloMapling {@code soloMapling.itemPool.UpgradeSimulator}。
 * 属依赖层（ItemUtilities/FreeMarket 管线调用 getEquipMarketValue），随本组一并移植。
 *
 * <p>适配点：
 * <ul>
 *   <li>{@code client.inventory.Equip / constants.inventory.EquipType / server.ItemInformationProvider}
 *       → gms 对应包；API 签名一致（gms scrollEquipWithId 返回 Item，忽略返回值即可）。</li>
 *   <li>{@code FreeMarket.EquipListGenerator.generateCleanItemEquip} →
 *       {@code org.gms.server.bot.BotLogic.generateCleanItemEquip}。</li>
 *   <li>{@code MapleVersionManager.getItemPoolVersion()} → {@code ServerConstants.VERSION}（v83 > 40，
 *       恒启用 30%/70% 卷轴，与源 v55 行为一致）。</li>
 * </ul>
 */
