        ...
        for (Map.Entry<EquipType, Double> entry : equipTypesWithSkipChance.entrySet()) {
            ...
            if (random.nextDouble() >= skipChance) {
                try {
                    ...
                    // cosmetic equip piece skipped; keep decorating the rest
                    log.debug("EquipDecoratorRules failed to equip type {} for bot {}",
                            type, fakechar != null ? fakechar.getId() : "null", e);
