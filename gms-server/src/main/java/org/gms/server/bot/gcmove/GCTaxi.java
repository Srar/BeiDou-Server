...
final class GCTaxi {
    ...
    // ADAPTATION: SoloMapling gated the cab network behind MapleVersionManager.isPortalinCurrentVersion
    // (a destination map not in the current server version could not be ridden to). gms has no such
    // version gate; treat every map as available (always true) until a gms-side gate is introduced.
    private static boolean isPortalInCurrentVersion(int mapId) {
        return true;
    }
}
