...
class BotMovementState {
    ...
    // 风暴抑制：safeTick 的连续失败计数与「已告警一次」标记。
    // 连续失败达阈值后 driver 自停该 entry，只 warn 一次，其余静默（debug）。
    volatile int consecutiveFailures = 0;
    volatile boolean failureWarned = false;
    ...
    /** 驱动 tick 的分阶段标记（仅诊断：safeTick 告警时指出失败阶段）。 */
    volatile String lastTickPhase = "init";
