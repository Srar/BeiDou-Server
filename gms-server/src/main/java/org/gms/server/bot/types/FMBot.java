...
import org.gms.server.bot.freemarket.HiredMerchantAdapter;
import org.gms.server.bot.freemarket.PlayerShopAdapter;
import org.gms.server.bot.freemarket.ShopKeeper;
...
import static org.gms.server.bot.BotLogic.isInsideFM;
import static org.gms.server.bot.BotLogic.isInsideFMRooms;
import static org.gms.server.bot.BotLogic.isPointNear;
import static org.gms.server.bot.commands.WarpCommands.FMRoomWarpPortalId;
...
public class FMBot extends BotSM {
    ...
    private void processRoom() {
        // gms 移植：源 getAllPlayerShops()/getAllHiredMerchants()；gms 商店以 MapObject 存储，
        // 此处按类型枚举收集。
        List<PlayerShop> shops = new ArrayList<>();
        List<HiredMerchant> merchants = new ArrayList<>();
        List<MapObject> objects = this.getChr().getMap().getMapObjectsInRange(
                new Point(0, 0), Double.POSITIVE_INFINITY,
                List.of(MapObjectType.SHOP, MapObjectType.HIRED_MERCHANT));
        for (MapObject obj : objects) {
            if (obj instanceof PlayerShop shop) {
                shops.add(shop);
            } else if (obj instanceof HiredMerchant merchant) {
    ...
    private void walkToShop(ShopKeeper shop) {
        ...
        try {
            ...
            if (shouldWalk) {
                walkChance = BASE_WALK_CHANCE;
                Point pos = getRandomizedPointXAxis(currentTarget.getPosition());
            ...
            log.debug("FMBot visit shop walk failed for bot {}", getChr() != null ? getChr().getId() : "null", ignored);
        }
        visitIndex++;
    }

    private Point getRandomizedPointXAxis(Point p) {
        // gms 移植：源 BotHelpers.getRandomizedPointXAxis 在目标点 X 轴随机抖动后返回；
        // 此处等价：X ± 10 内抖动，Y 保持不变。
        return new Point(p.x + Randomizer.nextInt(21) - 10, p.y);
    }
...
//            System.out.println("processShop ERROR. NO VALID MERCHANT / SHOP");
    ...
    private void purchaseShopItems() {
        ...
        for (int itemIndex = 0; itemIndex < items.size(); itemIndex++) {
            ...
            Integer itemMarketValue = BotEconomy.getItemMarketValue(thisItem);
...
//            System.out.println("ERROR. NO VALID MERCHANT / SHOP");
    ...
    private boolean exitRoom() {
        ...
        if (doorPoint != null
                && !isPointNear(this.getChr().getPosition(), doorPoint, 20)
                && doorWalkAttempts < MAX_DOOR_WALK_ATTEMPTS) {
            if (!GCMovement.isMoving(getChr())) { // not started yet, or the last attempt was abandoned
                doorWalkAttempts++;
    ...
    private Point getFMRoomDoorPortalPoint() {
        ...
        if (map == null) {
            log.warn("FMBot.getFMRoomDoorPortalPoint skipped: bot {} has no map", getChr().getId());
            return null;
        }
        Integer portalId = FMRoomWarpPortalId.get(roomNumber);
        if (portalId == null) {
            log.warn("FMBot.getFMRoomDoorPortalPoint skipped: no warp portal id for room {}", roomNumber);
            return null;
        }
        Portal portal = map.getPortal(portalId);
        if (portal == null) {
            log.warn("FMBot.getFMRoomDoorPortalPoint skipped: no portal {} on map {}", portalId, map.getId());
            return null;
        }
    ...
    public static boolean shouldPurchaseItem(Integer askingPrice, Integer marketValue) {
        ...
        for (int i = 0; i < thresholds.length; i++) {
            if (ratio <= thresholds[i]) {
                return Randomizer.nextDouble() < probabilities[i];
            }
        }
    ...
    public void updateState() {
        ...
        getDebugger().debugLoggingFull(String.format("%s FMBotState: %s", this.getChr().getName(), fmBotState), String.format("%s", fmBotState));
