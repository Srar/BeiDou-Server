...
import static org.gms.server.bot.decorate.JobPathLogic.determineBowmanPath;
import static org.gms.server.bot.decorate.JobPathLogic.determineThiefPath;
import static org.gms.server.bot.decorate.JobPathLogic.determineWarriorPath;

public class BotDecorateEquips {
    /**
     * Full decoration pass. Equip selection is an in-memory lookup through
     * {@link ItemInformationProvider}, so this is cheap enough to run inline at
     * spawn time. Everything runs sequentially on the calling thread - do NOT fan
     * out into async tasks, or multiple threads will mutate the same Character's
    ...
    static void equipTopBottom(Character fakechar) {
        int coatId = getCoatId(fakechar);
        int pantsId = getPantsId(fakechar);
        int longcoatId = getLongcoatId(fakechar);

        if (coatId > 0 && pantsId > 0) {
            BotCustomization.EquipBot(fakechar, coatId);
            BotCustomization.EquipBot(fakechar, pantsId);
        } else if (longcoatId > 0) {
    ...
    private static int getCoatId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.COAT, c);
    }

    private static int getPantsId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.PANTS, c);
    }

    private static int getLongcoatId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.LONGCOAT, c);
    }
...
//    }
    ...
    private static int getCapId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.CAP, c);
    }

    private static int getGlovesId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.GLOVES, c);
    }

    private static int getShoesId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.SHOES, c);
    }

    private static int getCapeId(Character c) {
        return ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.CAPE, c);
    }

    private static void equipIfPresent(Character c, int itemId) {
        if (itemId > 0) {
    ...
    private static void equipOutfit(Character character) {
        ...
        if (tryCoatAndPants) {
            ...
            int coatId = ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.COAT, character);
            if (coatId > 0) { // Assuming getRandomEquip returns <= 0 if no suitable equip is found
                ...
                int pantsId = ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.PANTS, character);
                if (pantsId > 0) {
...
//                BotCustomization.UnequipBot(character, coatId);
        ...
        int longcoatId = ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.LONGCOAT, character);
        if (longcoatId > 0) {
    ...
    public static void equipWeapon(Character character) {
        ...
        for (EquipType weaponType : possibleWeapons) {
            equipIfPresent(character, ItemInformationProvider.getInstance().getRandomEquipForWearing(weaponType, character));
        }

        // Equip shield if needed
        if (useShield) {
            equipIfPresent(character, ItemInformationProvider.getInstance().getRandomEquipForWearing(EquipType.SHIELD, character));
        }
    }
}
