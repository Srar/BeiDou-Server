...
//
// Ported 1:1 from SoloMapling (BotLogger / SocialCommands equivalents inlined).
@Slf4j
public class FollowerBot extends BotSM {
    ...
    private int leaderIdFromParty() {
        ...
        if (lead != null && !org.gms.server.bot.BotHelpers.isBot(lead)) {
            return lead.getId();
        }
    ...
    private Character firstRealMember(Party party) {
        for (PartyCharacter pc : party.getMembers()) {
            ...
            if (p != null && !org.gms.server.bot.BotHelpers.isBot(p) && p.getMap() != null) {
                return p;
            }
        }
        return null;
    ...
    public synchronized void stopScheduledTask() {
        ...
        log.info("[FollowerBot] stopped: " + (chr != null ? chr.getName() : "?"));
    }
}
