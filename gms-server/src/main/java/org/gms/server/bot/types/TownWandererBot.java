...
// Ported 1:1 from SoloMapling (SocialCommands / BotLogger / SoloMaplingUtilities equivalents inlined).
@Slf4j
public class TownWandererBot extends BotSM {
    ...
    protected void onScheduledStart() {
        // stopScheduledTask 退订全部事件；重启时重新订阅 LEVEL_UP（幂等）。
    ...
    private List<Integer> discoverTownFamily(int home) {
        ...
        try {
            ...
            log.debug("TownWandererBot.discoverTownFamily failed for bot {} (home {})",
                    getChr() != null ? getChr().getId() : "null", home, ignored);
        }
        return fam;
    }
    ...
    private void doRandomEmote() {
        if (rollChanceInverse(10)) {
            emote(getChr(), rollChanceInverse(2) ? 2 : 3);
        }
    }
    ...
    public synchronized void stopScheduledTask() {
        ...
        log.info("[TownWandererBot] stopped: " + (chr != null ? chr.getName() : "?"));
    }

    // ── SoloMaplingUtilities.rollChanceInverse / SocialCommands equivalents ──

    /** A 1-in-N dice roll (NOT a percent chance). */
    private boolean rollChanceInverse(int outOf) {
        return outOf <= 1 || rng.nextInt(outOf) == 0;
    }
