...
//        I love him/Ilove her/I miss her
    ...
    /**
     * T4-P6：描述文件行池静态缓存（key = classpath 资源路径）。源实现每次调用
     * {@link #getRandomStoreDescription} 都打开文件全量 reservoir 扫描，摆摊管线
     * 每摊多次调用造成重复 IO；缓存后 random.nextInt(size) 取一行与源 reservoir
     * 均匀抽样等价（源无缓存，gms 2 核增强）。加载失败/空文件缓存空列表，
     * 调用方降级为空字符串。
     */
     ...
     * BotHelpers 大池发放，不再经本类 {@link #getRandomCharacterIGN} 自动登记，
     * 若无人登记则 assignedCharacterNames 恒空、{@link #getRandomShopOwnerIGN}
     * 的 35%「复用在线 bot 名」分支成为死代码。
     */
    public static synchronized void registerAssignedCharacterName(String name) {
    ...
    private static List<String> loadAndShuffleNames() {
        ...
        try (InputStream in = FMShopDescGen.class.getResourceAsStream(filePath)) {
            if (in == null) {
        ...
        if (names.isEmpty()) {
            // T4-P5：空文件/资源缺失兜底固定占位名，保证 namePool 永不空——否则
            // getRandomIGN 的 namePool.get(0) 会抛 IndexOutOfBoundsException，整区摆摊静默失败。
            names = new ArrayList<>(List.of("Shop"));
        }
    ...
    protected static String getRandomStoreDescription(String type) {
        ...
        List<String> lines = DESC_LINE_CACHE.computeIfAbsent(filePath, FMShopDescGen::loadDescLines);
        if (lines.isEmpty()) {
