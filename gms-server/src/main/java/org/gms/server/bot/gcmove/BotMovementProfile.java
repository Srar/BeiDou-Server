...
record BotMovementProfile(int totalSpeedStat, int totalJumpStat, boolean snowShoes)
    ...
    static BotMovementProfile fromCharacter(Character character) {
        ...
        // 移植适配：gms Character 没有 SoloMapling 的 getTotalMoveSpeedStat()/getTotalJumpStat()，
        // 等价计算内联为本文件的 totalMoveSpeedStat()/totalJumpStat()（装备速度/跳跃 + BuffStat 加成，
        // 语义与原 SoloMapling client.Character 方法逐行一致）。
        int totalSpeed = speedBaseline + (totalMoveSpeedStat(character) - BASE_TOTAL_STAT);
        int totalJump = jumpBaseline + (totalJumpStat(character) - BASE_TOTAL_STAT);
    ...
    private static boolean wearsSnowShoes(Character character) {
        Inventory equipped = character.getInventory(InventoryType.EQUIPPED);
        if (equipped == null) {
            return false;
        }
        Item shoe = equipped.getItem((short) -7);
        if (shoe == null) {
            return false;
        }
    ...
    /* 移植适配：SoloMapling client.Character.getTotalMoveSpeedStat() 在 gms 不存在（lombok 生成的
     * getter 集不同），等价实现内联于此。原 SoloMapling 语义：基础 100 + 已装备 Equip.getSpeed() 之和
     * + BuffStat.SPEED 加成，下限 1。 */
    private static int totalMoveSpeedStat(Character character) {
        int total = 100;
        for (Item item : character.getInventory(InventoryType.EQUIPPED)) {
            if (item instanceof Equip equip) {
                total += equip.getSpeed();
            }
        }
        Integer speedBuff = character.getBuffedValue(BuffStat.SPEED);
        if (speedBuff != null) {
            total += speedBuff;
        }
    ...
    /* 移植适配：SoloMapling client.Character.getTotalJumpStat() 的等价内联实现，语义逐行一致。 */
    private static int totalJumpStat(Character character) {
        int total = 100;
        for (Item item : character.getInventory(InventoryType.EQUIPPED)) {
            if (item instanceof Equip equip) {
                total += equip.getJump();
        ...
        Integer jumpBuff = character.getBuffedValue(BuffStat.JUMP);
        if (jumpBuff != null) {
            total += jumpBuff;
        }
        return Math.max(1, total);
