...
/**
 * 移植自 SoloMapling {@code soloMapling.itemPool.ItemInformationProviderUtilities}。
 *
 * <p>适配点：
 * <ul>
 *   <li>{@code client.Character / client.Job / constants.inventory.EquipType / server.ItemInformationProvider}
 *       → gms 对应包（{@code org.gms.client / org.gms.constants.inventory / org.gms.server}）。</li>
 *   <li>DebugUtilities → gms no-op 等价物 {@code org.gms.server.bot.replay.DebugUtilities}。</li>
 *   <li>{@code SoloMaplingUtilities.random}（静态 Random）→ 类内私有 static Random，行为一致。</li>
 *   <li>EquipMetadataCache 查询 API：源 {@code get().query(eqType).nonCash().forGender(...).levelBetween(...)
 *       .forJobExact(...).asList()}，gms 版缓存为 {@code get().nonCash(eqType)}（已非 cash）+ 等价字段
 *       过滤（gender/reqLevel/reqJob），语义逐条对齐（见 gms EquipMetadataCache.EquipEntry）。</li>
 * </ul>
 */
public class ItemInformationProviderUtilities {
    ...
    // 源 static import soloMapling.server.SoloMaplingUtilities.random；gms 无此工具，等价内联。
...
//    }
    ...
    public static Integer getRandomEquip(EquipType eqType, int maxLevel, Job jobStyle, int gender) {
        ...
        List<EquipMetadataCache.EquipEntry> candidates = filterEquips(
                EquipMetadataCache.get().nonCash(eqType), gender, reqJob, minLevel, maxLevel);
        ...
        if (candidates.isEmpty()) {
            List<EquipMetadataCache.EquipEntry> belowCap = filterEquips(
                    EquipMetadataCache.get().nonCash(eqType), gender, reqJob, 0, maxLevel);
    ...
    /**
     * 等价于源 EquipMetadataCache 流式查询
     * {@code query(eqType).nonCash().forGender(gender).levelBetween(minLevel, maxLevel).forJobExact(reqJob)}
     * （minLevel 传 0 时等价于 {@code .maxLevel(maxLevel)}）。gms 版缓存的 nonCash(type) 已过滤 cash，
     * 此处按条目字段逐项过滤，语义与源一致。
     */
    private static List<EquipMetadataCache.EquipEntry> filterEquips(
            List<EquipMetadataCache.EquipEntry> source, int gender, int reqJob, int minLevel, int maxLevel) {
        List<EquipMetadataCache.EquipEntry> filtered = new ArrayList<>();
        for (EquipMetadataCache.EquipEntry e : source) {
            if (e.gender != 2 && e.gender != gender) continue; // forGender
            if (e.reqLevel < minLevel || e.reqLevel > maxLevel) continue; // levelBetween / maxLevel
            if (e.reqJob != reqJob) continue; // forJobExact
            filtered.add(e);
        }
        return filtered;
    }
