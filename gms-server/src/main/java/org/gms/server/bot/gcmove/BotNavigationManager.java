...
final class BotNavigationManager {
    ...
    private static void notifyWarmup(BotMovementState entry, Character bot) {
        ...
        long walkable = map.getFootholds().getAllFootholds().stream()
    ...
    private static int intraRegionTravelCost(BotNavigationGraph graph, int regionId, Point from, Point to) {
        ...
        if (region != null && region.isRopeRegion) {
            ...
            // 原引用 BotMovementManager.cfg.CLIMB_SPEED_PXS（P1-E）→ 复用 BotPhysicsEngine.cfg.CLIMB_SPEED_PXS（同值）
            return Math.max(0, (int) Math.round((travel * 1000.0) / Math.max(1, BotPhysicsEngine.cfg.CLIMB_SPEED_PXS)));
        }
    ...
    private static boolean canGrabRopeAtCurrentPosition(Point botPos, Rope rope) {
        // 原引用 BotMovementManager.cfg.ROPE_GRAB_X（P1-E）→ 复用 BotPhysicsEngine.cfg.ROPE_GRAB_X（同值）
        return Math.abs(botPos.x - rope.x()) <= BotPhysicsEngine.cfg.ROPE_GRAB_X
