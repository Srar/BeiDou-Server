...
final class GrindLoot {
    ...
    private static final long FFA_OWNER_PROTECT_MS = 15_000;
    ...
    // nearestCollectableDrop (identical collectable / settle / leash / ledge filters), used to plan a whole sweep.
    ...
    // no owner, or whose owner-protection window has expired). 1:1 等价于 DropCommands.botCanLoot。
    private static boolean collectable(Character chr, MapItem mapItem) {
    ...
    // Pick up one specific drop. 1:1 等价于 DropCommands.botLootSingleDrop。
    private static void pickUp(Character chr, MapItem mapItem) {
        if (mapItem == null || mapItem.isPickedUp()) {
            return;
        }
        chr.getMap().pickItemDrop(
                PacketCreator.removeItemFromMap(mapItem.getObjectId(), 2, chr.getId()), mapItem);
    }
