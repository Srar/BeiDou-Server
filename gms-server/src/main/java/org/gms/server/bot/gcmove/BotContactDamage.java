...
final class BotContactDamage {
    ...
    private static boolean isHostileLivingMonster(Monster monster) {
        return monster != null
                && monster.isAlive()
                && monster.getStats() != null
                && !monster.getStats().isFriendly(); // gms 适配：缺怪数据（stats==null）的怪跳过接触伤害，防止下游 getPADamage NPE
    }
    ...
    private static void scheduleAlertReset(BotMovementState entry) {
        ...
        Runnable reset = () -> {
            ...
            try {
                ...
                log.debug("BotContactDamage alert-reset broadcast failed for bot {}",
                        entry.bot != null ? entry.bot.getId() : "null", ignored);
            }
        };
        // 对齐源 MethodScheduler.runAfterDelay 的「定时器跑在共享调度池、慢体跳虚拟线程」不变式：
        // 调度池线程只负责触发，真正的 reset 体经 BotExecutors.runAsync 跳到虚拟线程，慢/阻塞体不会占用调度池。
        BotExecutors.schedule(() -> BotExecutors.runAsync(reset), delay);
    }
}
