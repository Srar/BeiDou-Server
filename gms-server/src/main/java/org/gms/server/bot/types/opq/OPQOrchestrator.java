...
import static org.gms.server.bot.types.opq.OPQConstants.CHAMBERLAIN_EAK;
...
public class OPQOrchestrator {
    ...
    // gms 移植：SoloMapling 用 ScheduledFuture 周期调度；gms BotExecutors 只有单发
    // schedule，此处以「自续期单发」实现 1Hz tick（tickRunning 门控停摆）。
    ...
    private Character resolveLeader() {
        ...
        MapleMap map = anchor.getChr().getMap();
        if (map == null) return null;
        return map.getChannelServer().getPlayerStorage().getCharacterById(leaderId);
    }

    public boolean isChamberlainSpawned() {
        Character leader = resolveLeader();
        if (leader == null || leader.getMap() == null) return false;
        return BotLogic.isNpcPresent(leader.getMap(), CHAMBERLAIN_EAK);
    }
    ...
    public void followLeaderWarp(Character fakechar, Point pt) {
        Character leader = resolveLeader();
        if (leader == null || leader.getMap() == null) return;

        try {
            // gms 移植：SoloMapling 的 BotGeneration.warpBotToLocation 未落地，改用
            // Character.changeMap(map, pos) 服务器端直传（等效无客户端 OPQ 传送）。
            fakechar.changeMap(leader.getMap(), pt);
        } catch (Exception e) {
