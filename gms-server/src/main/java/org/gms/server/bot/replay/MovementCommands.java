...
import org.gms.server.bot.replay.MovementPacket;
import org.gms.server.bot.replay.SingleMoveCommand;
import org.gms.server.bot.replay.MovementRecording;
import org.gms.server.bot.replay.navigation.NavigationElement;
...
import static org.gms.server.bot.BotLogic.isPointNear;
import static org.gms.server.bot.replay.InPacketReader.getMovementRecording;
import static org.gms.server.bot.replay.MovementPacketConstructor.createArtificialStopPacket;
import static org.gms.server.bot.replay.MovementPacketConstructor.createFallDownPacket;
import static org.gms.server.bot.replay.MovementPacketConstructor.createIdleStandlingPacket;
import static org.gms.server.bot.replay.MovementPacketConstructor.createSitPacket;
import static org.gms.server.bot.replay.MovementPacketConstructor.createUnsitPacket;
import static org.gms.server.bot.replay.MovementPacketConstructor.deconstructMovementInPacket;
import static org.gms.server.bot.replay.MovementPacketConstructor.modifyMovementPacketWithOffset;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.IDLE_LEFT;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.IDLE_RIGHT;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.JUMP_LEFT;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.JUMP_RIGHT;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.MOVING_LEFT;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.MOVING_RIGHT;
//import static soloMapling.ArtificialPlayer.BotMovementSystem.NavigationSystem.PathFinderbeta.getXYCoordsFromPacket;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.SIT_LEFT;
import static org.gms.server.bot.replay.MovementEnums.StanceValues.SIT_RIGHT;
import static org.gms.server.bot.BotHelpers.isBot;
import static org.gms.server.bot.replay.SingleMoveCommand.getConvertedStanceDirection;
import static org.gms.server.bot.replay.navigation.PathFinder.createPath;
...
public class MovementCommands {
    ...
    public static boolean BotMoveStreamHelper(MovementRecording mvr, Character fakechar,
        ...
        for (int i = 0; i < fullRecording.size(); i++) {
            ...
            if (nextMvp != null) {
                if (!waitBetweenTwoLong(mvp.getTimestamp(), nextMvp.getTimestamp())) {
...
//    }
    ...
    public static void BotIdleStandingUpdate(Character fakechar) {
        ...
        // gms 移植：SoloMapling 的 BotPerfStats.IDLE_BROADCASTS.increment() 为纯性能计数，未移植。
    ...
    public static void microTurnAroundToRight(Character fakechar) {
        ...
        try {
            ...
            log.debug("MovementCommands.microTurnAroundToRight failed for bot {}",
    ...
    // 1:1 移植自 SoloMapling BotHelpers.waitBetweenTwoLong（gms BotHelpers 未保留该方法）。
    // Returns false if the thread was interrupted (interrupt flag restored) so the replay loop can bail.
