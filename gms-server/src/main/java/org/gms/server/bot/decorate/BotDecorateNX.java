...
public class BotDecorateNX {
     ...
     * NOTE: SoloMapling's class-level javadoc claims S=[3,8]; the actual code
     * values are S=[2,5] and those are kept verbatim per porting rules.
     */
     ...
     * load time (from YAML + ItemInformationProvider auto-population), so per-bot
     * calls are just in-memory RNG. Safe to call from spawn-time code.
     */
