...
public class BlackjackDealerBot extends BotSM {
    ...
    private void dprint(String msg) {
        log.debug("[BlackjackDealerBot:{}] {}", getChr().getName(), msg);
    }

    private void faceTowards(Character target) {
        if (target == null || target.getPosition() == null || getChr().getPosition() == null) {
            return;
        }
        GCMovement.enable(getChr());
        GCMovement.face(getChr(), target.getPosition().x < getChr().getPosition().x);
    }
    ...
    public void updateState() {
        ...
        switch (table.getPhase()) {
            ...
            case PAYOUT:
                ...
                break;
            default:
                log.info("Unexpected blackjack phase: " + table.getPhase());
    ...
    private void handlePlayerTurn() {
        ...
        dprint("PLAYER_TURNS: idx=" + table.getCurrentPlayerIndex()
                ...
                + ", hand=" + currentPlayer.getHand() + ", isBot=" + BotHelpers.isBot(currentPlayer.getCharacter()));
    ...
    private void handleBust(BlackjackPlayer player, String name, int handValue) {
        ...
        BotGameSupport.lootItemListOnFloor(getChr(), player.getCardsOnMap());
    ...
    private void handleAutoStand(BlackjackPlayer player, String name) {
        ...
        BotGameSupport.botSpeak(getChr(), name + ": 21!");
    ...
    private void processPlayerAction(BlackjackPlayer player) {
        ...
        switch (player.getStatus()) {
            ...
            case BUST:
                ...
                break;
            default:
                log.info("Unknown player status: " + player.getStatus());
                break;
        }
    }
    ...
    private void handleResolving() {
        ...
        if (!lines.isEmpty()) {
            BotGameSupport.botSpeak(getChr(), String.join(" | ", lines));
        }
    ...
    private void dealCardToPlayer(BlackjackPlayer player) {
        ...
        MapItem cardOnMap = BotGameSupport.botThrowItemNoExpireOwnerOnly(getChr(), cardItemId, pos);
    ...
    private void processAllPlayersBets() {
        ...
        for (int i = 0; i < table.getPlayerCount(); i++) {
            ...
            if (items.isEmpty()) {
                ...
                betLines.add(BotLogic.announceBetString(chr, items));
            }
        }
        if (!betLines.isEmpty()) {
            BotGameSupport.botSpeak(getChr(), String.join(" | ", betLines));
        }
        for (Character chr : toKick) {
            kickPlayer(chr);
        }
    ...
    private Character payoutOrCollect(BlackjackPlayer player) {
        ...
        switch (player.getResult()) {
            ...
            case PUSH:
                ...
                return null;
            default:
                log.info("Unknown hand result for " + player.getName());
                player.setBetsProcessed(true);
                return null;
        }
    }
    ...
    private void payWinner(BlackjackPlayer player, boolean blackjackBonus) {
        boolean isAI = BotHelpers.isBot(player.getCharacter());
    ...
    private Character lootLoserBets(BlackjackPlayer player) {
        Character chr = player.getCharacter();
        double lootRadius = BotHelpers.isBot(chr) ? 1500 : 5000;
        ...
        if (potCollectible) {
            BotGameSupport.lootItemListOnFloor(getChr(), player.getBets());
            return null;
        }
    ...
    private void triggerArtificialPlayerBets() {
        ...
        for (int i = 1; i < table.getPlayerCount(); i++) {
            BlackjackPlayer player = table.getPlayer(i);
            if (!BotHelpers.isBot(player.getCharacter())) continue;
    ...
    private void triggerAIPlayerReaction(BlackjackPlayer player, String dialogueNode) {
        if (!BotHelpers.isBot(player.getCharacter())) return;
            ...
            BotTiming.after(1250 + org.gms.util.Randomizer.nextInt(1500), () -> {
                ...
                if (dialog != null && !dialog.getDialogue().isEmpty()) {
                    ...
                    if (emote > 0) {
                        ...
                        BotGameSupport.blockingSleep(400);
                    }
                }

                BotGameSupport.botSpeak(botChr, spokenLine);
    ...
    private void processBlackjackMessages(BlackjackPlayer player) {
        try {
            ...
            log.info("[Blackjack] Message: " + message.getSender().getName() + ":" + message.getContent());
    ...
    public void displayCommands(Character chr) {
        ...
        BotGameSupport.displayPlayerChatCommands(chr, List.of("Join"));
    }

    // Shown to a real player when it becomes their turn.
    private void showPlayerActionHint(Character chr) {
        BotGameSupport.displayPlayerChatCommands(chr, List.of("Stand", " - Hit -"));
    }
