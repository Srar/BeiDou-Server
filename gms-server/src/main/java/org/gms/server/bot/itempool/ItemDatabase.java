...
import static org.gms.server.bot.itempool.ItemSelector.ITEM_POOL_VERSION;
import static org.gms.server.bot.itempool.ItemSelector.loadYamlResource;
import static org.gms.server.bot.itempool.ItemSelector.toInteger;
import static org.gms.server.bot.itempool.ItemSelector.toIntegerList;
import static org.gms.server.bot.itempool.ItemSelector.toPriceMap;
import static org.gms.server.bot.itempool.ItemSelector.toTierMap;

/**
 * 移植自 SoloMapling {@code soloMapling.itemPool.ItemDatabase}。
 *
 * <p>适配点：
 * <ul>
 *   <li>yamlbeans → snakeyaml + classpath 资源（见 {@link ItemSelector} 类头说明）：
 *       池文件由 {@code src/main/java/.../itemConfig/*.yaml} 迁到
 *       {@code resources/org/gms/server/bot/itempool/itemConfig/}。</li>
 *   <li>MapleVersionManager.getItemPoolVersion() → {@link ItemSelector#ITEM_POOL_VERSION}（=55）。
 *       池数据最高版本键为 72（chair.yaml），版本常量对齐源 MapleVersionManager=55 语义
 *       （55 以上条目可见），与 ServerConstants.VERSION=83 无关。</li>
 *   <li>processItem 中 tier/price 键值宽容转换（snakeyaml 原生类型 vs 源 yamlbeans 全 String）。</li>
 * </ul>
 */
public class ItemDatabase {
    ...
    public static void loadItemDatabase() throws Exception {
        ...
        for (String itemPool : itemPools) {
            Map<String, Object> itemPoolData = loadYamlResource("itemConfig/" + itemPool);

            for (Map.Entry<String, Object> entry : itemPoolData.entrySet()) {
                String itemType = entry.getKey();
                List<Map<String, Object>> itemList = (List<Map<String, Object>>) (List<?>) entry.getValue();
    ...
    public void processItem(Map<String, Object> itemMap) {
        ...
        List<Integer> variantIds = toIntegerList(itemMap.get("variant_id"));

        // Convert tier map
        Map<Integer, String> tiers = toTierMap(itemMap.get("tier"));

        // Convert price map
        Map<Integer, Integer> prices = toPriceMap(itemMap.get("price"));

        Object testNode = itemMap.get("success_rate");
        if (testNode != null) {
            int successRate = toInteger(itemMap.get("success_rate"));
            int statBonus = toInteger(itemMap.get("stat_bonus"));
    ...
    public String getItemTier(int variantId) {
        return getItemTier(variantId, ITEM_POOL_VERSION);
    }
    ...
    public Integer getItemPrice(int variantId) {
        return getItemPrice(variantId, ITEM_POOL_VERSION);
    }
    ...
    public boolean checkIfItemExistsInCurrentVersion(int itemId) {
        ...
        if (tierMap != null) {
            ...
            boolean isInVersion = ITEM_POOL_VERSION >= earliestVersion;
            return isInVersion;
        }
        return false;
    }
