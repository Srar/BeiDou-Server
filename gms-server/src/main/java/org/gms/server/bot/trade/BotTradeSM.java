...
import org.gms.server.bot.BotBlockList;
...
import static org.gms.server.bot.trade.BotTradeCommands.getTradePartnerCharacter;
...
//        }
    ...
    protected boolean postItemsForSale() {
        ...
        if (itemForSale.getInventoryType() == InventoryType.EQUIP) {
...
//        }
    ...
    /** 等价 SocialCommands.BotEmote：面部表情广播。 */
    private static void botEmote(Character chr, int emote) {
        if (chr != null && chr.getMap() != null) {
            chr.getMap().broadcastMessage(chr, PacketCreator.facialExpression(chr, emote), true);
            chr.getMap().broadcastMessage(chr, PacketCreator.facialExpression(chr, emote), chr.getPosition());
        }
    }

    /** 等价 FMEconomyManager.formatPriceToShorthand(int)（1 位小数）。 */
    private static String formatPriceToShorthand(int price) {
        if (price < 1000) {
        ...
        String formatted = String.format("%.1f", formattedPrice);
