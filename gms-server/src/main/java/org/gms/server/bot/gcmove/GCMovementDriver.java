...
final class GCMovementDriver {
    ...
    // 风暴抑制：单 bot 连续失败达该次数后 driver 自停（map 已 dispose / 服务器下线等不可恢复场景）。
    ...
    private static void safeTick(BotMovementState entry) {
        try {
            // gms 移植：SoloMapling 的 BotPerfStats.MOVEMENT_TICKS.increment() 为纯性能计数，未移植。
            tick(entry);
            // 成功一拍即复位失败计数与告警标记，避免历史偶发失败累积到自停阈值。
            ...
            // gms 增强：源实现完全静默（吞异常不打日志），生产排障无任何线索。
            // 风暴抑制：单 entry 连续失败达阈值后自停 driver，只 warn 一次，其余静默（debug），
            // 避免同图多 bot 反复抛异常刷屏。
            ...
            if (entry.consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                ...
                if (!entry.failureWarned) {
                    ...
                    log.warn("Bot movement tick failed {} consecutive times for bot {} (map={}, lastMap={}) — driver stopped",
                            entry.consecutiveFailures,
                            entry.bot != null ? entry.bot.getId() : "null",
                            entry.bot != null && entry.bot.getMap() != null ? entry.bot.getMapId() : -1,
                            entry.lastMapId, t);
                }
                return;
            }
            log.debug("Bot movement tick failed for bot {} (map={}, lastMap={})",
                    entry.bot != null ? entry.bot.getId() : "null",
                    entry.bot != null && entry.bot.getMap() != null ? entry.bot.getMapId() : -1,
                    entry.lastMapId, t);
        }
    ...
    private static void tick(BotMovementState entry) {
        ...
        entry.lastTickPhase = "tickMobDamage";
        ...
        entry.lastTickPhase = "resolveTarget";
        ...
        entry.lastTickPhase = "stepMovementCore";
