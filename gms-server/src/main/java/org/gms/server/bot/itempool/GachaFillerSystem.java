...
public class GachaFillerSystem {
    ...
    // 源：src/main/java/soloMapling/itemPool/itemConfig/gachaFiller.yaml。
    // gms 按包相对 classpath 资源加载（与 DesirableEquipList 一致）：
    // GachaFillerSystem.class.getResourceAsStream 相对本类包解析为
    // /org/gms/server/bot/itempool/itemConfig/gachaFiller.yaml。
    private static final String YAML_PATH = "itemConfig/gachaFiller.yaml";
    ...
    private static void loadGachaData() throws Exception {
        // 源用 com.esotericsoftware.yamlbeans.YamlReader；gms 依赖 snakeyaml，加载语义等价。
        Object object = null;

        InputStream in = GachaFillerSystem.class.getResourceAsStream(YAML_PATH);
        if (in != null) {
            try {
                object = new Yaml().load(in);
            } finally {
                in.close();
            }
        } else {
