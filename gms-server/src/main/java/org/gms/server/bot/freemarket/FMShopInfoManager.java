...
/**
 * 移植自 SoloMapling FreeMarket.FMShopInfoManager（坐标表与区域映射逐行照搬）。
 *
 * <p>移植说明（gms 底座差异）：
 * <ul>
 *   <li>源类通过 {@code import static soloMapling.FreeMarket.ArtificialShopGenerator.random}
 *       共享 ArtificialShopGenerator 的静态 Random；ArtificialShopGenerator 属后续移植波次，
 *       为保持本类自包含、不依赖并行代理半成品，改为类内私有 {@link #random}（行为等价，
 *       仅偏移量 ±5 的均匀随机）。</li>
 *   <li>源中未使用的 {@code java.util.Map} import 已移除。</li>
 *   <li>4 区域列表及 henesys 24 点 / ludi 28 点 / perion 26 点 / elnath 27 点坐标
 *       与源完全一致；未引用 EnvironmentManager.FM_ROOM_MAP_IDS，保持解耦。</li>
 * </ul>
 */
public class FMShopInfoManager {

    private static final Random random = new Random(); // 移植自 ArtificialShopGenerator.random 的等价物
