package org.gms.server.bot.gacha;
...
import static org.gms.server.bot.itempool.GachaFillerSystem.getRandomMesoGachaFiller;
...
public class CustomReactor {
    ...
    public static void getAllReactorsData(Character fakechar) {
        ...
        for (Reactor reactor : reactors) {
            if (reactor.getState() < 4 && reactor.isAlive()) {
                // SoloMapling DebugUtilities.debugprint 是编译期禁用的 no-op，此处以 log.debug 等价。
                log.debug("Reactor: {} {} {} {} {} {}", reactor.getObjectId(), reactor.getId(), reactor.getPosition(), reactor.isAlive(), reactor.isActive(), reactor.getState());
            }
        }
    }
    ...
    public static void dropItemAtReactor(MapleMap map, int oid, int itemId, Character owner) {
        ...
        if (ItemConstants.getInventoryType(itemId) != InventoryType.EQUIP) {
            ...
            drop = ItemInformationProvider.getInstance().getEquipById(itemId);
            if (drop == null) {
                log.warn("CustomReactor.dropItemAtReactor: no equip data for item {}; skipping", itemId);
                return;
            }
        }
    ...
    public static void hitReactor(MapleMap map, int oid) {
        Reactor reactor = map.getReactorByOid(oid);
        if (reactor == null) {
            log.warn("CustomReactor.hitReactor: reactor {} not found", oid);
            return;
        }
        byte state = reactor.getState();
        state++;
        reactor.setState(state);
        map.broadcastMessage(PacketCreator.triggerReactor(reactor, 0));
    }
    ...
    private static void notDelayedReactorDrops(Character owner, List<ReactorDropEntry> drops, Reactor reactor, Point dropPos, int posX) {
        ...
        for (ReactorDropEntry d : drops) {
            ...
            if (d.itemId == 0) {
                // gms 的 World#getMesoRate 经 lombok 返回 float（源 SoloMapling 为 int）。
                int mesoDrop = NumberTool.floatToInt(1000 * owner.getWorldServer().getMesoRate());
                ...
                if (ItemConstants.getInventoryType(d.itemId) != InventoryType.EQUIP) {
                    ...
                    if (drop == null) {
                        log.warn("CustomReactor: no equip data for item {}; skipping drop", d.itemId);
                        continue;
                    }
                }
    ...
    private static void delayedReactorDrops(Character owner, List<ReactorDropEntry> drops, Reactor reactor, Point dropPos) {
        final float worldMesoRate = owner.getWorldServer().getMesoRate();
        ...
        for (ReactorDropEntry d : drops) {
            ...
            if (d.itemId == 0) {
                ...
                if (ItemConstants.getInventoryType(d.itemId) != InventoryType.EQUIP) {
                    ...
                    ItemInformationProvider ii = ItemInformationProvider.getInstance();
                    Item equip = ii.getEquipById(d.itemId);
                    if (equip == null) {
                        log.warn("CustomReactor: no equip data for item {}; skipping drop", d.itemId);
                        continue;
                    }
    ...
    private static void delayedReactorDropsStandard(Character owner, List<ReactorDropEntry> drops, Reactor reactor, Point dropPos, int posX) {
        ...
        for (ReactorDropEntry d : drops) {
            if (d.itemId == 0) {
                int mesoDrop = NumberTool.floatToInt(1000 * worldMesoRate);
    ...
    private static void dropFromReactorCustom(MapleMap map, int oid, List<ReactorDropEntry> drops, Character owner, boolean delayed) {
        Reactor reactor = map.getReactorByOid(oid);
        if (reactor == null) {
            log.warn("CustomReactor.dropFromReactorCustom: reactor {} not found", oid);
            return;
        }
        Point reactorPos = reactor.getPosition();
        if (reactorPos == null) {
            log.warn("CustomReactor.dropFromReactorCustom: reactor {} has no position", oid);
            return;
        }
        int posX = (int) reactorPos.getX();
        int posY = (int) reactorPos.getY();
        boolean meso = true;
        int mesoChance = 10;
