package org.gms.server.bot.wander;
...
public final class BotWanderSystem {
    ...
    // gms 移植：源用 ExecutorServiceManager.getScheduledExecutorService() 拿周期池；gms 的 BotExecutors
    // 只暴露一次性 schedule，周期调度走共享 TimerManager.register（与 BotTickService / BotEquipChecker
    // 同款），stop() 经 TimerManager.stop 取消。
    ...
    public static void stop(Character bot) {
        ...
        if (w != null && w.task != null) {
            TimerManager.getInstance().stop(w.task);
        }
    ...
    private static void startInternal(Character bot, boolean banded, int bandLo, int bandHi) {
        ...
        w.task = TimerManager.getInstance().register(() -> {
            try {
                tick(w);
            } catch (Throwable ignored) {
                // a thrown tick would cancel the periodic task; swallow and re-evaluate next poll (self-healing 400ms tick)
                log.debug("BotWanderSystem tick failed for bot {}", w.bot != null ? w.bot.getId() : "null", ignored);
            }
        }, POLL_MS, initialDelay);
