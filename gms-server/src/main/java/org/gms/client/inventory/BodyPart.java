...
/**
 * 装备槽位枚举（参考 SoloMapling 的 client.inventory.BodyPart 1:1 移植）。
 * <p>
 * gms 的已装备物品槽位编码与 SoloMapling 一致：普通装备槽为 {@code -(value)}，
 * 现金装备槽为 {@code -(value + CASH_BASE)}（见 BotLogic.getSlotDstValueByEquipSlotName）。
 * 本枚举仅作为 BotLogic 等 bot 支撑类的槽位命名入口，不改动 gms 现有装备存储。
 */
